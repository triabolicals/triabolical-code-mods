...
pub fn level_up_window_setup_hook(this: &LevelUpWindowController, unit: &Unit, next: &Unit, method_info: OptionalMethod){
    ...
    if GameVariableManager::get_bool(LEVEL_DIS_KEY) {
        if unit.internal_level!= 0 {
            ...
            unsafe {
                try_set_text_string(this.level, level_str, None);
            }
        }
    }
}
#[skyline::hook(offset = 0x1f9d320)]
pub fn unit_info_set_level_hook(this: &UnitInfoParamSetter, unit: Option<&Unit>, x: i32, z: i32, selected_god: bool, god: &GodUnit, method_info: OptionalMethod){
            ...
            unsafe {
                ...
                if result { 
                    if p.internal_level >= 0 {
                        displayed_level = enhance_level+ (p.internal_level as i32);
                    }
                 }
                try_set_text(this.level, displayed_level, None);
            }
        },
        None => {},
    }
...
impl ConfigBasicMenuItemSwitchMethods for  GrowthMod {
    ...
    extern "C" fn set_help_text(this: &mut ConfigBasicMenuItem, _method_info: OptionalMethod){
        this.help_text = Mess::get("MID_GAMESTART_GROWMODE_SELECT_HELP");
    }
    extern "C" fn set_command_text(this: &mut ConfigBasicMenuItem, _method_info: OptionalMethod){
        let mode =  GameVariableManager::get_number(GROWTH_KEY);
        let growth_mode = GameUserData::get_grow_mode();
        if mode == 0 {
            if growth_mode == 1 { this.command_text = Mess::get("MID_SYS_Grow_Fixed"); }
            else { this.command_text = Mess::get("MID_SYS_Grow_Random"); }
        }
        else if mode == 1 { 
            if growth_mode == 0 {  this.command_text =  Mess::get("MID_SYS_Grow_Fixed"); }
            else { this.command_text =  Mess::get("MID_SYS_Grow_Random"); }
        }
...
impl ConfigBasicMenuItemSwitchMethods for LevelMod {
    ...
    extern "C" fn set_help_text(this: &mut ConfigBasicMenuItem, _method_info: OptionalMethod){
        if GameVariableManager::get_bool(LEVEL_DIS_KEY) { this.help_text = "Displays unit's total level.".into(); } 
        else { this.help_text = "Default level display.".into(); }
    }
    extern "C" fn set_command_text(this: &mut ConfigBasicMenuItem, _method_info: OptionalMethod){
        if GameVariableManager::get_bool(LEVEL_DIS_KEY) { this.command_text = on_str(); }
