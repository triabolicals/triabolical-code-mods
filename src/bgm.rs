...
pub fn patch_imm_bgm(){
    if GameVariableManager::get_bool(BGM_KEY) { Patch::in_text(0x0228d414).bytes(&[0x01, 0x00, 0x80, 0x52]).unwrap(); }
    else { Patch::in_text(0x0228d414).bytes(&[0xe1, 0x03, 0x13, 0x2a]).unwrap(); }
}
