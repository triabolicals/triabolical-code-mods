...
pub fn reliance_can_level_up( unit_a: &Unit, unit_b: &Unit, method_info: OptionalMethod) -> bool {
    ...
    if result {
        let support = GameVariableManager::get_number(SUPPORT_KEY);
        if support == 2 || support == 3 {
            unsafe {
                reliance_level_up(unit_a, unit_b, None);
                let reliance = unit_reliance_try_get(unit_a, unit_b, None);
                if reliance.is_some(){
                    if reliance.unwrap().level == 3 { add_a_reliance_count(None); }
                    else if reliance.unwrap().level == 2 { add_b_reliance_count(None); }
...
pub fn unit_reliance_try_get(unit_a: &Unit, unit_b: &Unit, method_info: OptionalMethod) -> Option<&'static UnitRelianceData>;
...
pub fn update_reliances(){
    ...
    unsafe {
        ...
        if support == 2 || support == 3 {
            ...
            println!("{} supports are leveled up", count);
        }
    }
}
